...
def visualize_network(x, p, ax=None, title=None, show_edges=True, edge_threshold=1e-6,
    ...
    for i in range(m):
        # Check if genotype is extinct
        is_extinct = populations[i] == 0.0 or populations[i] < 1e-10
        
        if is_extinct:
            # Extinct genotype - grey and faded but still visible
            facecolor = '#CCCCCC'  # Medium grey
            edgecolor = '#666666'  # Dark grey
            alpha = 0.6  # More visible
            # Use small fixed size for extinct
            radius = np.sqrt(node_scale * 0.5) / 50  # Bigger than before
        else:
            # Alive genotype - normal coloring
            facecolor = node_colors[i]
            edgecolor = 'black'
            alpha = 0.85
            radius = np.sqrt(node_sizes[i]) / 50
        
        circle = Circle(positions[i], radius=radius,
                       facecolor=facecolor, edgecolor=edgecolor,
                       linewidth=2, alpha=alpha, zorder=10)
        ax.add_patch(circle)
        
        # Mark extinct with BIG RED X
        if is_extinct:
            ax.plot(positions[i, 0], positions[i, 1], 'x', 
                   color='red', markersize=20, markeredgewidth=4, zorder=15)
    ...
    n_surviving = np.sum(populations > 0)  # Changed from > 0.01 * total_pop to > 0
...
def live_evolution_demo(p, C_range=(0.1, 5.0), n_frames=30, interval=300):
    ...
    def update(frame):
        ...
        # Kill off susceptible genotypes at high antibiotic
        if C > 2.5:
            # Genotypes below median IC50 die out
            median_IC50 = np.median(IC50)
            for j in range(m):
                if IC50[j] < median_IC50:
                    # Susceptible genotypes die exponentially with increasing C
                    death_rate = (C - 2.5) / (C_range[1] - 2.5)  # 0 to 1
                    x[j] *= np.exp(-10.0 * death_rate)  # Increased from -8.0 to -10.0
                    if x[j] < 0.05:  # Increased threshold from 0.01
                        x[j] = 0.0  # Complete extinction
        
        # At high C, kill off even moderately resistant (increased aggression)
        if C >= 3.0:  # Changed from > 3.5 to >= 3.0
            threshold = 0.05 * x[:m].max()  # Increased from 0.03
            x[:m] = np.where(x[:m] < threshold, 0.0, x[:m])
